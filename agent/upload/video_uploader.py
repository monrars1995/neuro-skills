...
class VideoUploadManager:
    ...
    def add_to_queue(self, video_path: Path, creative_name: str,
        ...
        Returns:
            ID do upload
        """
        upload_id = f"upload_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{creative_name}"        self.uploads[upload_id] = {
            "id": upload_id,
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "uploading": sum(1 for u in self.uploads.values() if u["status"] == "uploading"),
            "completed": sum(1 for u in self.uploads.values() if u["status"] == "completed"),
...
class BatchUploader:
    ...
    def upload_creatives_from_folder(self, folder_path: Path,
                                      campaign_name: str,
        ...
        threads = self.upload_manager.start_uploads(
            progress_callback=lambda uid, prog, up, total: self._on_progress(
                uid, prog, up, total, results, progress_callback
    ...
    def _on_progress(self, upload_id: str, proportion: float, uploaded: int,
                     total: int, results: Dict, callback: Callable):
